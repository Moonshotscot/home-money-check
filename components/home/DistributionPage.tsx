...
const configs: Record<DistributionKind, DistributionConfig> = {
  staff: {
    ...
    introduction: "Home Money Check gives every member of your team access to a free gas, electricity and broadband bills check.",
  ...
  introducer: {
    ...
    introduction: "Give your clients access to a free gas, electricity and broadband check. Home Money Check speaks with them, builds their quote and explains every saving.",
  ...
  partner: {
    ...
    title: "Give your audience their own Home Money Check page.",
...
export function DistributionPage({ kind }: { kind: DistributionKind }) {
  ...
  const titleAccent = {
    staff: "save them money.",
    introducer: "save money on household bills.",
    partner: "Home Money Check.",
  }[kind];
        ...
        <section className="relative isolate overflow-hidden px-5 pb-14 pt-9 sm:px-8 sm:pb-14 sm:pt-10 lg:pb-16 lg:pt-11">
          ...
          <div className="mx-auto grid max-w-[86rem] items-center gap-11 lg:grid-cols-[0.94fr_1.06fr] lg:gap-14">
            <div>
              ...
              <h1 className="display-font mt-6 max-w-[13ch] text-[clamp(3.5rem,5.2vw,5.9rem)] leading-[0.88] tracking-[-0.06em] text-[#3D145F]">{titleStart}<span className="text-[#D89F00]">{titleAccent}</span>{titleEnd}</h1>
              <p className="mt-6 max-w-xl text-lg font-semibold leading-8 text-[#5D5062] sm:text-[1.15rem]">{page.introduction}</p>
              <p className="mt-5 max-w-xl border-l-4 border-[#F0C646] pl-5 text-lg font-extrabold leading-7 text-[#3D145F]">{page.promise}</p>
